...
For a while now I have been enamored by the question of "what is venom?" It is a fairly nuanced question that will undoubtebly have a nuanced answer, but today I have decided to describe  trends in what has been called a "venom" to this date.
...
The data I used is available [here](http://blog.tijeco.info/data/venom.txt).
...
```{r}
library(tidyverse)
venom_dates <- read.table("~/tijeco_blog/data/venom.txt",header = T)
```
...
Now we just need to do some minor adjustments to the data to prepare it for plotting. Since the data is literally just a list of years, it is best that we convert it to a table describing the frequncies that each year occured. This will correspond to number of venom proteins uploaded in a given year.  

The other lines is probably a bit clunky, but it gets the job done, in a somewhat concise matter. 
```{r}
